...
Page({
  ...
  getOrder: function(page, status) {
    ...
    var url = 'api/Order/GetStoreOrderList?page=' + page + '&pageSize=10&status=' + status + '&storeId=0&marketId=' +that.data.marketId
      ...
      if (res.data.flag) {
        ...
        if (res.data.list != null) {
          for (var i = 0; i < newlist.length; i++) {
            newlist[i].CouponMoney = newlist[i].CouponMoney.toFixed(2);
            newlist[i].CurrencyCouponMoney = newlist[i].CurrencyCouponMoney.toFixed(2);
            newlist[i].DeliveryCost = newlist[i].DeliveryCost.toFixed(2);
            newlist[i].PayMoney = newlist[i].PayMoney.toFixed(2);
            var StoreDetail = newlist[i].StoreDetail;
            for (var j = 0; j < StoreDetail.length; j++) {
              for (var k = 0; k < StoreDetail[j].detail.length; k++) {
                StoreDetail[j].detail[k].Price = StoreDetail[j].detail[k].Price.toFixed(2);
              } 
            }

            newlist[i].StoreDetail = StoreDetail
          }
        }
  ...
  orderDeliver:function(e){
    ...
    if (that.data.IsRten){
      ...
      var orderNo = e.currentTarget.dataset.delid;
      console.log("orderNo=" + orderNo);

      app.request('api/Order/DeliverOrder?orderNo=' + orderNo, {}, "GET",
        function (res) {
          console.log("获取");
  ...
  orderNewDeliver: function (e) {
    ...
    if (that.data.IsRten) {
      ...
      app.request('api/Order/NewDeliverOrder?orderNo=' + orderNo, {}, "GET",
        function (res) {
          console.log("获取");
          console.log(res.data.flag);
          if (res.data.flag) {
